...
class Grid {
  ...
  private ArrayList<CellIndex> getAdjacentCells(CellIndex index) {
  ...
  private ArrayList<CellIndex> getAdjacentCells(CellIndex index, boolean[][][] processedForMaze, Status status) {
    ArrayList<CellIndex> adjacent = getAdjacentCells(index);
  ...
  private void generateMazeData() {
    ...
    Set<CellIndex> toBeProcessed = new HashSet<CellIndex>(getAdjacentCells(start));
